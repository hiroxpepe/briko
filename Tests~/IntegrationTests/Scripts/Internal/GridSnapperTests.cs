...
namespace Briko.Tests.Internal {
    ...
    /// <summary>
    /// Tests for GridSnapper - 0.5m grid snapping.
    /// </summary>
...
#nullable enable
        ...
        public void Snap_AlreadyOnGrid_ReturnsSameValue() {
            float[] result = GridSnapper.Snap(raw: new float[] { 0.0f, 0.5f, 1.0f }, grid_unit: 0.5f);
            Assert.That(result[0], Is.EqualTo(0.0f).Within(0.001f));
            Assert.That(result[1], Is.EqualTo(0.5f).Within(0.001f));
            Assert.That(result[2], Is.EqualTo(1.0f).Within(0.001f));
        }

        [Test, Description("Snap rounds up to next grid unit when above midpoint")]
        public void Snap_AboveMidpoint_RoundsUp() {
            float[] result = GridSnapper.Snap(raw: new float[] { 0.3f, 0.0f, 0.0f }, grid_unit: 0.5f);
            Assert.That(result[0], Is.EqualTo(0.5f).Within(0.001f));
        }

        [Test, Description("Snap rounds down to previous grid unit when below midpoint")]
        public void Snap_BelowMidpoint_RoundsDown() {
            float[] result = GridSnapper.Snap(raw: new float[] { 0.2f, 0.0f, 0.0f }, grid_unit: 0.5f);
            Assert.That(result[0], Is.EqualTo(0.0f).Within(0.001f));
        }
