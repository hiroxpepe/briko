...
namespace Briko.Tests.Convention {
    ...
    ///
    /// Convention (root ns = "Briko"):
    ///   ConventionScan.TARGET_DIRS[0]/&lt;A&gt;/&lt;B&gt;/Foo.cs  ->  namespace Briko.&lt;A&gt;.&lt;B&gt;
    ///   - files directly in the source root map to "Briko" itself.
    ...
    public class NamespaceFolderConventionTests
    {
        ...
        static readonly ConventionRule ScriptsRule =
            new(ConventionScan.TARGET_DIRS[0], "Briko.Editor", new string[] {  });
        ...
        static string? first_namespace(string path)
        {
        ...
        static List<string> scan_for_mismatches(string root_dir, ConventionRule rule, bool strict)
        {
            ...
            foreach (var path in Directory.EnumerateFiles(root_dir, "*.cs", SearchOption.AllDirectories)) {
                ...
                var actual = first_namespace(path);
