...
namespace Briko.Tests.Convention {
    /// <summary>
    /// Roslyn gate: calls to our own types (Briko.*) must pass arguments by
    ...
    public class NamedArgumentConventionTests
    {
        static bool is_own(ISymbol? s)
        {
            ...
            return asm == null || asm.StartsWith("Briko") || s.Locations.Any(l => l.IsInSource);
        }
        ...
        public void OwnCalls_UseNamedArguments()
        {
            ...
            var comp = CSharpCompilation.Create("Briko.Scan", trees.Values, refs,
